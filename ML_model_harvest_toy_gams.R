...
smp_size <- floor(0.96 * nrow(df.harvest))

train_ind <- sample(seq_len(nrow(df.harvest)), size = smp_size)

train_data <- df.harvest[train_ind,1:7]
train_labels <- df.harvest[train_ind,8]

test_data <- df.harvest[-train_ind,1:7]
test_labels <- df.harvest[-train_ind,8]
...
subDir <- "ML_harvest25_envi_soil_10_mse_75_50_1_sigmoid_tensorboard"
...
build_model <- function() {
  ...
  model <- keras_model_sequential() %>%
    layer_dense(units = 75, activation = "sigmoid", input_shape = dim(train_data)[2]) %>%
...
postscript(file="Learning curve.eps",horiz=TRUE,onefile=FALSE,width=8.5,height=11,paper="letter")
